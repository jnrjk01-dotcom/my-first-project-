...
@media screen and (max-width: 767px) {
  ...
  /* Webflow left translate(0, 40px) on the camera as the opening state of an interaction
     ...
     It offsets the section for no reason, and a transform on a sticky element's ancestor
     also interferes with how it sticks, so it goes. */
  ...
  /* The section holds still the way the original template did it: the camera sticks, and
     the trigger above is a tall spacer for it to stick through. The spacer's height is
     set by the script, since it depends on how far the cards have to travel.

     Sticky rather than a GSAP pin so the sideways travel can begin while the section is
     still rising and the lock can happen underneath it. A pin cannot do that: it owns
     both the lock and the animation's range, so nothing moves until it engages. */
  .dcc-svc-pinned .scroll-mobile_trigger {
    height: auto;
